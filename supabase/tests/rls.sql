...
insert into public.games (id, week_id, external_id, home_team, away_team, kickoff_at, moneyline_home, moneyline_away, total, over_odds, under_odds, line_source)
values
  ('aaaaaaaa-0000-4000-8000-000000000001', 999, 'rls-test-game-1', 'CIN', 'BAL', '2026-09-13T17:00:00Z', -180, 155, 44.5, -110, -110, 'demo'),
  ('aaaaaaaa-0000-4000-8000-000000000002', 999, 'rls-test-game-2', 'KC',  'DEN', '2026-09-13T20:05:00Z', -350, 280, 42.5, -110, -110, 'demo')
on conflict (id) do nothing;
...
-- (total_pick, moneyline_pick).
-- Expected: permission denied for table picks
begin;
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"11111111-1111-1111-1111-111111111111","role":"authenticated"}', true);
update public.picks set total_correct = true, moneyline_correct = true
...
set local role authenticated;
select set_config('request.jwt.claims','{"sub":"11111111-1111-1111-1111-111111111111","role":"authenticated"}', true);
update public.picks set total_pick = 'UNDER', moneyline_pick = 'BAL'
where user_id = '11111111-1111-1111-1111-111111111111'
